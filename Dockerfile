...
RUN ln -s /usr/bin/clang-3.9 /usr/bin/clang && ln -s /usr/bin/clang++-3.9 /usr/bin/clang++
COPY . /rethinkdb
VOLUME [ "/rethinkdb" ]
WORKDIR /rethinkdb
RUN ./configure --allow-fetch CXX=clang++ --fetch protoc --fetch npm && make -j4 DEBUG=1 && make install
